...
put /Users/shuzhan/Desktop/redis-7.2-rc1.tar.gz /opt
tar -zxvf redis-7.2-rc1.tar.gz

# 2. 安装
cd /opt/redis-7.2-rc1
make && make install
...
# 6. 查看redis版本
redis-server -v
Redis server v=7.1.240 sha=00000000:0 malloc=jemalloc-5.2.1 bits=64 build=3cf6e6a23e89149b
...
- 官方redis的docker中是不带配置文件的，redis配置取默认值
- [官网下载](https://redis.io/download)一个redis，将redis下面的 **redis.conf** 文件拷贝出来
- 自定义修改，利用容器数据卷挂载到redis中的docker中

```bash
bind 127.0.0.1 -::1     # 限制redis只能本地cli访问， 必须注释掉该行
protected-mode no       # 默认yes，开启保护模式，限制为本地访问
daemonize no            # 守护进程，默认no，改为yes会使配置文件方式启动redis失败
databases 10            # 默认为16，数据库个数，可以改成10验证配置文件是否生效
```

```bash
# 1. mac控制台上传文件到centos服务器中opt文件, 首先进入该文件的目录
scp redis.conf root@60.205.229.31:/opt/
# 2. 将宿主机中的配置文件挂载到 docker中的redis，这样启动的容器中就会有一个redis.conf 文件
docker pull redis
docker run -v /opt:/usr/local/etc/redis -p 6381:6379  -d --name szredis redis redis-server /usr/local/etc/redis/redis.conf
# /opt:/usr/local/etc/redis： opt为装有redis.conf 文件的宿主机目录
# /usr/local/etc/redis/redis.conf:  新生成的容器中会有这个redis这个目录，如不配置则不会有 
