...
```java
/** 二叉树的层序列遍历：从顶到底，每层按照从左到右封装结果
 *  LeetCode-102 */
public List<List<Integer>> levelOrder(TreeNode root) {
    return get(root, true);
}

/** 二叉树的层序列遍历二：从底到顶，每层按照从左到右封装结果
 * LeetCode-107 */
public List<List<Integer>> levelOrderBottom(TreeNode root) {
    return get(root, false);
}

private List<List<Integer>> get(TreeNode root, boolean flag) {
...
```java
/** 二叉树的层序列锯齿遍历：从上到小，按照一层左右，一层右左的顺序
