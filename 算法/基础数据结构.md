...
# 索引原则：索引0开始存储节点数据，i表示索引
- 节点i的父节点为 floor((i-1)/2),     当i大于0时
- 节点i的左子节点为： 2i+1            结果不能大于size
- 节点i的右子节点为：   2i+2            结果不能大于size
```

![image-20231022152920357](https://erick-typora-image.oss-cn-shanghai.aliyuncs.com/img/image-20231022152920357.png)
...
```bash
# 根据原来数组，重新构建大顶堆数组
- 入堆新元素，加入到数组末尾，索引为0，则直接添加
- 不断比较新加元素与其父节点的值
    - 父节点更小，则父节点向下移动，继续向上找parent
    - 直到父节点值更大
    
# 相同元素，如果添加顺序不同，则构建的大顶堆也不同
- 主要因为大顶堆的左右元素的关系不确定
```

![image-20231022164829005](https://erick-typora-image.oss-cn-shanghai.aliyuncs.com/img/image-20231022164829005.png)

```java
class MaxHeap {

    /*威廉姆斯算法：准备新的堆，逐个添加元素*/
    public int[] heapifyFirst(int[] arr) {
    ...
    private void put(int data, int childIndex, int[] arr) {
        int parentIndex = (childIndex - 1) / 2;
        while (arr[parentIndex] < data && childIndex > 0) {
