...
class MinHeap {
      ...
      private void up(int childIndex) {
          ...
          //有效子索引(最后面情况)，父>子
          while (parentIndex > -1 && arr[parentIndex] > arr[childIndex]) {
      ...
      private void down() {
          ...
          int rightIndex = 2;
          while (true) {
              // 找到最小的孩子结点,保证leftIndex和rightIndex都必须小于size
              ...
              if (minIndex == parentIndex) { // minIndex没变，表示找到头了
                  break;
              }
...
```bash
- 1. 将数组元素依次放入堆中，堆满后
    - 1.1 如果新元素比堆顶大，则替换堆顶元素，并堆顶元素下潜
    - 1.2 如果新元素比堆顶小或等，则什么也不做
- 2. 返回堆顶元素 
```
